...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    // Progress bar ticker (updates every 100ms based on actual time elapsed)
  ...
  const heroHeading = (
    <div className="max-w-md w-full mx-auto">
      <h1 className="text-4xl md:text-6xl font-bold mb-6 flex flex-wrap gap-2 items-center">
        <span className="text-yellow drop-shadow-[4px_4px_0_rgba(0,0,0,1)]">is</span>
        <span className="text-white drop-shadow-[4px_4px_0_rgba(0,0,0,1)]">claude</span>
        <span className="text-black bg-yellow px-2 y2k-border rotate-2">down</span>
        <span className="text-white drop-shadow-[4px_4px_0_rgba(0,0,0,1)]">again?</span>
      </h1>
      <p className="mb-8 font-bold text-lg">get instantly notified via email when anthropic claude goes down (or comes back up).</p>
    </div>
  );

  return (
    <section className="grid grid-cols-1 md:grid-cols-2 min-h-[calc(100vh-76px)]">
          ...
          <div className="y2k-card p-6 bg-white relative">
            ...
            {step === "email" && (
              <form onSubmit={handleSendOtp} className="flex flex-col gap-4">
                <div>
                  <label htmlFor="subscribe-email" className="block font-bold mb-2">your email address</label>
                  <input
                    id="subscribe-email"
                    type="email"
                    required
                ...
                <button type="submit" disabled={loading} className="y2k-button w-full mt-2">
            ...
            {step === "otp" && (
              <form onSubmit={handleVerify} className="flex flex-col gap-4">
                <div>
                  <label htmlFor="subscribe-otp" className="block font-bold mb-2">enter 6-digit otp</label>
            ...
            {step === "success" && (
              <div className="text-center py-6">
                <h2 className="font-bold text-2xl mb-2 text-green">subscribed!</h2>
                <p className="text-sm">you're all set. we'll email you the moment claude changes status.</p>
                <div className="mt-6 y2k-pill bg-yellow text-black font-bold py-2 px-4 inline-block shadow-none border-2">
                  monitoring active
                </div>
              </div>
            )}
